...
class Series(object):
    ...
    def __init__(self,
                 ...
                 q_value=None):
        '''
        input at initialization:
        
        date_string - descriptor string, typically the day we did the experiment
        T_function - a function returned from RegularGridInterpolator, that can interpolate the temperature
        d_value - average diameter of droplets in m
        d_sigma - standard deviation of diameter of droplets in m
        s_value - average spacing of droplets in m
        s_sigma - standard deviation spacing of droplets in m
        q_value - flow rate in microliters/minute
        '''
    ...
    def set_observables(self,
        ...
        '''
        input:
        frequency - never actually used
        distances - array of distances in m 
        counts - array of counts of droplets in focus
        fractions - array of fractions of ice
        ...
        self.ideal_volume = self.calculate_volume(d_value)
        ...
        # calculate ideal J:
        self.ideal_J_at_calculated = [self.calculate_J(self.q_value,
                                         self.s_value,
                                         self.d_value,
        ...
        self.ideal_J = self.calculate_J_gradient(self.q_value,
                                         self.s_value,
                                         self.d_value,
                                         self.measured_distances,
                                         self.fractions)
    ...
    def calculate_J_gradient(self, q, s, d, distances,fractions):
        '''
        Gradient method
        ...
        this is for an expression straight from observabes, meaning that we 
        calcualte using distance instead of time
        '''
        ...
        the_gradient=np.gradient(fractions_log,distances,edge_order=1)
        
        factor = np.divide((-36*np.multiply(q_m3_per_s, s)),
                           (np.multiply(np.power(np.pi, 2), np.power(d, 6))))

        J = np.multiply(factor, the_gradient)
    ...
    def run_MC_for_only_T(self, trials=1000 ,do_J=False):
        '''        
        ...
        if do_J:
            self.J_MC_matrix_at_measured = np.zeros([trials, len_calculated])
        ...
        for trial in range(trials):
            ...
            if do_J:
                trial_J_array=self.calculate_J_gradient(trial_q, trial_s, 
                                                                       trial_d, self.measured_distances,trial_f_array)
            ...
            if do_J:
                self.J_MC_matrix_at_measured[trial,:] = trial_J_array
    ...
    def save_column_file(self,path):
    ...
    def plot_J_vs_T(self, ax=None, **kwargs):
        if ax == None:
            ...
            ax = plt.gca()
        xerr = np.array([self.low_T_err, self.high_T_err])
        yerr = np.array([self.low_J_err_at_calculated, self.high_J_err_at_calculated])

        ax.scatter(self.ideal_T_at_calculated,
                   self.ideal_J_at_calculated,
                   **kwargs)
        kwargs['label'] = None
        ax.errorbar(self.ideal_T_at_calculated,
                    self.ideal_J_at_calculated,
